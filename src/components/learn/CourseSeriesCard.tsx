...
import {
    Layers, Clock, Award, ChevronRight, Play, FileText, Code,
    Terminal, CheckCircle, Users, MessageSquare
} from "lucide-react";
...
const FEATURE_MAP: Record<string, any> = {
    video: { label: "Videos", icon: Play, color: "text-rose-500", route: "course-series" },
    reading: { label: "Readings", icon: FileText, color: "text-amber-500", route: "default" },
    sandbox: { label: "Sandboxes", icon: Code, color: "text-blue-500", route: "interactive-project" },
    project: { label: "Projects", icon: Terminal, color: "text-purple-500", route: "interactive-project" },
    quiz: { label: "Quizzes", icon: CheckCircle, color: "text-green-500", route: "assessment" },
    capstone: { label: "Capstone", icon: Award, color: "text-orange-500", route: "capstone" },
    community: { label: "Community", icon: Users, color: "text-indigo-500", route: "community" },
    feedback: { label: "Feedback", icon: MessageSquare, color: "text-pink-500", route: "feedback" },
};
            ...
            <div className="p-5 flex-1 flex flex-col space-y-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                    {skills.slice(0, 3).map((skill) => (
                        <span key={skill} className="px-2 py-0.5 text-[10px] uppercase font-semibold tracking-wider bg-muted text-muted-foreground rounded">
                            {skill}
                        </span>
                    ))}
                </div>
                ...
                <div>
                    <p className="text-xs font-medium text-muted-foreground mb-2 uppercase tracking-wide">Includes</p>
                    <div className="flex flex-wrap gap-2">
                        {includes.slice(0, 5).map(key => {
                            ...
                            return (
                                <Link key={key} href={`/coming-soon/${feature.route}?id=${id}`}>
                                    <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-muted/50 hover:bg-primary/10 hover:text-primary transition-colors cursor-pointer" title={feature.label}>
                        ...
                        {includes.length > 5 && (
                            <span className="text-xs text-muted-foreground self-center">+{includes.length - 5} more</span>
                        )}
                    </div>
                </div>
